...
async def main():
    ...
    async def generate_typescript(args):
        ...
        try:
            ...
            if crud_scaffolds:
                ...
                response_text += f"\n\n{'='*60}"
                response_text += "\n⛔⛔⛔ ABSOLUTE CONSTRAINTS — VIOLATIONS WASTE 30-40 SECONDS EACH ⛔⛔⛔"
                response_text += f"\n{'='*60}"
                response_text += "\n"
                response_text += "\nPERFORMANCE: Write(full file) = 30-40 SECONDS. Edit(targeted change) = 2-3 SECONDS."
                response_text += "\n"
                ...
                response_text += "\n"
                response_text += "\n2. index.css — Write FORBIDDEN. Use Edit to EXTEND :root (oklch only, keep @theme inline)."
                response_text += "\n"
                response_text += "\n3. Layout.tsx — Write FORBIDDEN. Use Edit for APP_TITLE/APP_SUBTITLE only."
                response_text += "\n"
                response_text += "\n4. CRUD files — Do NOT touch (*Page.tsx, *Dialog.tsx, ConfirmDialog, StatCard, PageShell)."
                response_text += "\n"
                response_text += "\n5. ALL files below are IN FULL — do NOT use Read on any of them."
                response_text += "\n"
                response_text += "\nSTEPS (in order):"
    ...
    options = ClaudeAgentOptions(
        system_prompt={
            ...
            "preset": "claude_code",
            "append": (
                "\n\n## ⛔ ABSOLUTE TOOL CONSTRAINTS — VIOLATIONS WASTE 30-40 SECONDS EACH\n\n"
                "PERFORMANCE: Write(full file) = 30-40 SECONDS. Edit(targeted change) = 2-3 SECONDS.\n\n"
                "### DashboardOverview.tsx\n"
                "- FIRST TIME ONLY: Write is allowed (initial creation).\n"
                "- AFTER FIRST WRITE: Write is FORBIDDEN on this file. Use ONLY Edit tool.\n"
                "- If npm build fails → use Edit to fix the specific error lines. Do NOT rewrite.\n"
                "- ❌ WRONG pattern: Write → Read → Write (wastes 60+ seconds!)\n"
                "- ✅ RIGHT pattern: Write ONCE → Edit for any subsequent fix\n\n"
                "### index.css\n"
                "- Write is FORBIDDEN. ONLY use Edit to extend :root with new tokens.\n\n"
                "### Layout.tsx\n"
                "- Write is FORBIDDEN. ONLY use Edit for APP_TITLE/APP_SUBTITLE.\n\n"
                "### CRUD scaffold files (*Page.tsx, *Dialog.tsx, ConfirmDialog, StatCard, PageShell)\n"
                "- Write AND Edit are FORBIDDEN. These files are complete.\n\n"
                "### After ANY Write or Edit\n"
                "- Do NOT Read the file back. Do NOT Read files from tool responses. Move to the next task.\n"
            )
        },
